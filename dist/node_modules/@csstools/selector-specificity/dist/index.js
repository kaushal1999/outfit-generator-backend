...
function selectorSpecificity(s) {
  ...
  if ("id" === s.type) t += 1;else if ("tag" === s.type) i += 1;else if ("class" === s.type) c += 1;else if ("attribute" === s.type) c += 1;else if (isPseudoElement(s)) {
    if ("::slotted" === s.value.toLowerCase()) {
    ...
    case ":nth-last-child":
      if (c += 1, s.nodes && s.nodes.length > 0) {
        const o = s.nodes[0].nodes.findIndex(e => "tag" === e.type && "of" === e.value.toLowerCase());
        if (o > -1) {
          const n = [e.selector({
            nodes: s.nodes[0].nodes.slice(o + 1),
            value: ""
          })];
          s.nodes.length > 1 && n.push(...s.nodes.slice(1));
          const a = specificityOfMostSpecificListItem(n);
          t += a.a, c += a.b, i += a.c;
        }
      }
      break;
...
export { compare, selectorSpecificity };